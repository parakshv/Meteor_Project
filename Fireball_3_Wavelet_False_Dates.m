...
for x=1:10
    wave_new(1:size(yW),x*2-1:(x*2))=filter(b(x,:),a(x,:),yW); 
end
...
for x=1:20
    mean_wave(1,x)=mean(wave_new(1:size(yW),x)); 
end
...
for x=1:20
    wave_new_mean_corrected(:,x)=wave_new(:,x)-mean_wave(1,x); 
end
...
for x=1:20
        ...
        RMS(y,x)=rms(wave_new_mean_corrected(startpoint:endpoint,x));
        end
end
...
if site_num==1
    ...
    datasheet=Fireball_Detection_US000Y_Wavelet_Data_False_Dates;
elseif site_num==2
    load('Fireball_Detection_US0016_Wavelet_Data_False_Dates.mat')
    datasheet=Fireball_Detection_US0016_Wavelet_Data_False_Dates;
elseif site_num==3
    load('Fireball_Detection_US0015_Wavelet_Data_False_Dates.mat')
    datasheet=Fireball_Detection_US0015_Wavelet_Data_False_Dates;
end
