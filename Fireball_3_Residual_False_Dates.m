...
for x=1:10
    resi_new(1:size(residual),x*2-1:(x*2))=filter(b(x,:),a(x,:),residual); 
end
...
for x=1:20
    mean_resi(1,x)=mean(resi_new(1:size(residual),x)); 
end
...
for x=1:20
    resi_new_mean_corrected(:,x)=resi_new(:,x)-mean_resi(1,x); 
end
...
for x=1:20
        ...
        RMS(y,x)=rms(resi_new_mean_corrected(startpoint:endpoint,x));
        end
end
...
xtime=(length(residual)-1)/100000;
...
if site_num==1
    ...
    datasheet=Fireball_Detection_US000Y_Residual_Data_False_Dates;
elseif site_num==2
    load('Fireball_Detection_US0016_Residual_Data_False_Dates.mat')
    datasheet=Fireball_Detection_US0016_Residual_Data_False_Dates;
elseif site_num==3
    load('Fireball_Detection_US0015_Residual_Data_False_Dates.mat')
    datasheet=Fireball_Detection_US0015_Residual_Data_False_Dates;
end
...
for i=1:n
    if datasheet(i).File_Name(7:37)==meteor_filename(7:37);
        duplicate=1;
        break
    else
        duplicate=0; 
...
if duplicate==0;
    ...
    datasheet(n+1).Time_of_Event=append(finalmeteortrue_test,' ','UTC');
    datasheet(n+1).Duration=duration;
    %NS first and then EW is the order for the variables below
    
    for i=1:10 
        datasheet(n+1).(fn{i*3+1})=rms_baseline_and_during(1,i*2-1:i*2);
        datasheet(n+1).(fn{i*3+2})=rms_baseline_and_during(2,i*2-1:i*2);

        datasheet(n+1).(fn{i*3+3})=datasheet(n+1).(fn{i*3+2})./datasheet(n+1).(fn{i*3+1});
    end
