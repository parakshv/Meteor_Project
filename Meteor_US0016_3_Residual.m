...
%Currently hardcoded to show NS column of residual by choosing column 1
...
%Currently hardcoded for just the rms of residual_0_2_new_mean_corrected, in
...
for i=1:n
    if Meteor_Detection_US0016_Residual_Data(i).File_Name(7:37)==meteor_filename(7:37);
        duplicate=1;
        break
    else
        duplicate=0; 
...
if duplicate==0;
    ...
    Meteor_Detection_US0016_Residual_Data(n+1).Time_of_Event=append(finalmeteortrue_test,' ','UTC');
    Meteor_Detection_US0016_Residual_Data(n+1).Duration=duration;
    %NS first and then EW is the order for the variables below
    
    for i=1:10 
        Meteor_Detection_US0016_Residual_Data(n+1).(fn{i*3+1})=rms_baseline_and_during(1,i*2-1:i*2);
        Meteor_Detection_US0016_Residual_Data(n+1).(fn{i*3+2})=rms_baseline_and_during(2,i*2-1:i*2);

        Meteor_Detection_US0016_Residual_Data(n+1).(fn{i*3+3})=Meteor_Detection_US0016_Residual_Data(n+1).(fn{i*3+2})./Meteor_Detection_US0016_Residual_Data(n+1).(fn{i*3+1});
    end
