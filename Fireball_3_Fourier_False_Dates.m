...
for x=1:10
    four_new(1:size(yF),x*2-1:(x*2))=filter(b(x,:),a(x,:),yF); 
end
...
for x=1:20
    mean_four(1,x)=mean(four_new(1:size(yF),x)); 
end
...
for x=1:20
    four_new_mean_corrected(:,x)=four_new(:,x)-mean_four(1,x); 
end
...
for x=1:20
        ...
        RMS(y,x)=rms(four_new_mean_corrected(startpoint:endpoint,x));
        end
end
...
if site_num==1
    ...
    datasheet=Fireball_Detection_US000Y_Fourier_Data_False_Dates;
elseif site_num==2
    load('Fireball_Detection_US0016_Fourier_Data_False_Dates.mat')
    datasheet=Fireball_Detection_US0016_Fourier_Data_False_Dates;
elseif site_num==3
    load('Fireball_Detection_US0015_Fourier_Data_False_Dates.mat')
    datasheet=Fireball_Detection_US0015_Fourier_Data_False_Dates;
end
